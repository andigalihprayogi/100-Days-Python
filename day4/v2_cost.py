...
total_gas_daily = 0
eat_everyday = 50_000
apartement = 1_000_000
apartement_total = 0
eat_everyday_price = 0
...
for i in range(733):
    ...
    total_price += eat_everyday
    eat_everyday_price += eat_everyday
    ...
    if (i+1) % 30 == 0:
        total_price += apartement
        apartement_total += apartement
        print('test')
...
minimal_income = round(total_price/733, 2)
print(f'minimal uang yang harus didapat dalam sehari = {locale.currency( minimal_income, grouping=True )}, '
      f'total harga kos kosan = {locale.currency( apartement_total, grouping=True )}'
      f'total makan sehari hari = {locale.currency( eat_everyday_price, grouping=True )}')
